...
echo git clone https://github.com/zcnet4/build.git build
git clone https://github.com/zcnet4/build.git build
...
"%PYTHON%" build/gyp.py -Dtarget_arch=%target_arch% -Duv_library=%library% -D"component=shared_library" skynet/skynet.gyp
...
:help
echo gen_build.bat [debug/release] [test/bench] [clean] [noprojgen] [nobuild] [x86/x64] [static/shared]
echo Examples:
echo   gen_build.bat              : builds debug build
echo   gen_build.bat test         : builds debug build and runs tests
echo   gen_build.bat release bench: builds release build and runs benchmarks
goto exit
